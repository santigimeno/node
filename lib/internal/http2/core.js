...
const onClientRequestCreatedChannel = dc.channel('http2.client.request.created');
const onClientRequestStartChannel = dc.channel('http2.client.request.start');
const onClientRequestErrorChannel = dc.channel('http2.client.request.error');
const onClientResponseFinishChannel = dc.channel('http2.client.response.finish');
const onServerRequestStartChannel = dc.channel('http2.server.request.start');
const onServerResponseErrorChannel = dc.channel('http2.server.response.error');
const onServerResponseFinishChannel = dc.channel('http2.server.response.finish');
  ...
  _destroy(err, callback) {
    ...
    if (err) {
      if (session[kType] === NGHTTP2_SESSION_CLIENT) {
        ...
        if (onServerResponseErrorChannel.hasSubscribers) {
          onServerResponseErrorChannel.publish({
            stream: this,
            error: err
          });
        }
