...
function setupChannel(target, channel) {
  ...
  target.on('internalMessage', function(message, handle) {
    ...
    if ((message.cmd === 'NODE_HANDLE_ACK') ||
      ...
      if (message.cmd === 'NODE_HANDLE_ACK') {
        ...
        if (target._pendingMessage) {
          if (target._pendingMessage.retransmissions++ === 3) {
            target._pendingMessage.handle.close();
            target._pendingMessage = null;
            process.emitWarning('Handle could not be received, discard it');
          }
        }
      }
    ...
    // If the handle didn't reach the receiving process correctly retry up to 3
    // times. If it still fails discard and print a warning
  ...
  target._disconnect = function() {
    ...
    if (this._pendingMessage) {
      this._pendingMessage.handle.close();
